    ...
    return jsonify([
        {
            ...
            "otp_enabled": bool(row["otp_secret"])
        }
        for row in users
    ])
...
def delete_user_api(user_id):
    ...
    current_user = db.execute('SELECT id FROM users WHERE username = ?', (current_username,)).fetchone()
    if current_user and current_user['id'] == user_id:
        return jsonify({"error": "Eigenen Account kann man nicht löschen"}), 400
    user_count = db.execute('SELECT COUNT(*) FROM users').fetchone()[0]
    if user_count <= 1:
        return jsonify({"error": "Mindestens ein Benutzer muss bestehen bleiben"}), 400
...
def reset_user_password_api(user_id):
    ...
    if result.rowcount == 0:
        return jsonify({"error": "Benutzer nicht gefunden"}), 404
    log_activity(db, "update", "user", user_id)
    db.commit()
    return jsonify({"status": "updated"}), 200
